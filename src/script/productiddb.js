...
let counter = 1000; // Start counter for generating unique productId

// Async function to upload data
(async () => {
  for (const product of products) {
    try {
      // Ensure the product has a productId, if not, generate one
      if (!product.productId) {
        product.productId = String(counter++); // Convert productId to a string
      }
      ...
      console.log(`Uploaded: ${response.name} with productId: ${product.productId}`);
    } catch (error) {
